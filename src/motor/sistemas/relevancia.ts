...
export function imoveisParaVoce(v: Vida, modo: 'aluguel' | 'venda'): { para: Relevante<OfertaImovel>[]; resto: OfertaImovel[] } {
  ...
  // O alvo não é o mais barato: é o que cabe no jeito de viver (quanto da renda vai para a casa).
  const fracao = { apertado: 0.16, modesto: 0.23, confortavel: 0.28, folgado: 0.33 }[estilo];
  const alvo = modo === 'aluguel' ? Math.max(500, renda * fracao) : Math.max(60000, renda * fracao * 180 + tem * 0.8);
  const candidatas = cabem.filter(x => x.acessivel && x.o.quartos >= precisa && (!pets || x.o.aceitaPet) && !(x.o.modeloId === 'republica' && (i >= 30 || junto)));
  const perto = [...candidatas].sort((a, b) => Math.abs(Math.log((modo === 'aluguel' ? a.o.aluguel : a.o.preco) / alvo)) - Math.abs(Math.log((modo === 'aluguel' ? b.o.aluguel : b.o.preco) / alvo)));
  const melhor = perto[0]?.o;
  const segunda = perto[1]?.o;
  const lista: Relevante<OfertaImovel>[] = cabem.map(({ o, acessivel }) => {
    ...
    if (o === melhor) dar(2.4, modo === 'aluguel' ? 'Cabe bem no orçamento, do jeito que você vive.' : 'Uma compra do tamanho da sua renda.');
    else if (o === segunda) dar(1.6, modo === 'aluguel' ? 'Também cabe no orçamento.' : 'Outra que cabe na renda.');
    if (o.quartos >= precisa && precisa >= 2) dar(1.4, precisa >= 3 ? 'Um quarto para cada filho.' : 'Cabe a família.');
    if (o.quartos > precisa + 1) pontos -= modo === 'aluguel' ? 1.2 : 0.4;
    if (o.quartos < precisa) pontos -= 3;
    if (pets && o.aceitaPet) dar(0.6, 'Aceita animais.');
    if (modo === 'aluguel' && o.modeloId === 'republica') pontos += i < 26 && !junto ? 1.2 : i >= 30 ? -4 : -0.5;
    if (modo === 'aluguel' && o.modeloId === 'republica' && i < 26 && !junto) dar(0.1, 'Para começar, dividindo a casa.');
    if (modo === 'aluguel' && o.modeloId === 'kitnet' && i < 28 && !junto) dar(0.8, 'Um primeiro lugar só seu.');
    if ((estilo === 'confortavel' || estilo === 'folgado') && m.padrao >= 4) dar(0.8, 'Do jeito que você gosta de viver.');
    if (estilo === 'apertado' && m.padrao <= 2) dar(0.6, 'Simples e barato.');
    if (modo === 'venda' && o.estado === 'reforma') dar(0.3, 'Mais barato — mas vem com obra.');
